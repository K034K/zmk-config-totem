...
static void indicate_connectivity_internal(void) {
...
#if IS_ENABLED(CONFIG_ZMK_BLE)
    {
        ...
        // Blink blue (profile_index + 1) times to indicate which BT profile is selected
        uint8_t num_blinks = profile_index + 1;
        LOG_INF("Profile %d selected, blinking %s %d times", profile_index,
                color_names[CONFIG_RGBLED_WIDGET_PROFILE_COLOR], num_blinks);

        struct blink_item profile_blink = {
            .duration_ms = CONFIG_RGBLED_WIDGET_PROFILE_BLINK_MS,
            .color = CONFIG_RGBLED_WIDGET_PROFILE_COLOR,
            .sleep_ms = CONFIG_RGBLED_WIDGET_PROFILE_BLINK_MS,
        };
        struct blink_item profile_last_blink = {
            .duration_ms = CONFIG_RGBLED_WIDGET_PROFILE_BLINK_MS,
            .color = CONFIG_RGBLED_WIDGET_PROFILE_COLOR,
        };
        ...
        for (int i = 0; i < num_blinks; i++) {
            if (i < num_blinks - 1) {
                ...
                k_msgq_put(&led_msgq, &profile_last_blink, K_NO_WAIT);
            }
        }
    }
#endif
